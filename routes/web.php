...
use App\Http\Controllers\SearcController;
...
Route::group(
    ...
    function () {
        ...
        Route::get('/discover', [\App\Http\Controllers\FrontContoller::class, 'discover'])
            ->name('discover');
        ##################################################################################
        Route::get('/show_stats/{name_en}', [\App\Http\Controllers\FrontContoller::class, 'stats'])
            ->name('stats');
        ##################################################################################
        ...
        ##################################################################################
        Route::get(
            'dashboard/admin/join_front',
            [\App\Http\Controllers\PageController::class, 'join']
        )->name('front.join');

        Route::delete('/dashboard/admin/join_front/{id}', [PageController::class, 'destroy'])->name('join.destroy');

        Route::get(
            'dashboard/admin/contact_front',
        ...
        Route::get(
            'dashboard/admin/send_front',
            [\App\Http\Controllers\PageController::class, 'send']
        )->name('front.send');
        ##################################################################################
        ##################################################################################

        Route::get('/search', [SearcController::class, 'index'])->name('search');
    }
);
